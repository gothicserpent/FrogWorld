...
//protected List<string> allItems = new List<string>(new string[] { "FrogCoinBronze", "FrogCoinSilver", "FrogCoinGold" });
// run this on start - to try to get the ini files read from

void Start()
{

//fill the inventory from the ini file
...
private void InstantiateResource(string path, float x = 0.0f, float y = 0.0f, int quantity = 1)
{
	...
	instance.GetComponent<PersistentInventoryPickableItem>().Quantity = quantity;
}
